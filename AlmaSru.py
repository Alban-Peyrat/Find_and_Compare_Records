...
class AlmaSru(object):
    ...
    def get_datas(self) :
        ...
        for record in self.result.findall("sru:records/sru:record",ns):
            ...
            if record.find(".//sru:recordData/marc:record/marc:datafield[@tag='AVE']",ns):
                ...
                titre_pour_recouv = self.get_record_match_key_title(record)
                autre_forme_du_titre = self.get_varying_form_of_title(record)
                ...
                match_result = {
                    ...
                    'titre_pour_recouv' : titre_pour_recouv,
                    'autre_forme_du_titre' : autre_forme_du_titre,
                    'date_pub' : date_pub,
    ...
    def get_record_match_key_title(self ,record):
        key_title = ''
        for subfield in record.find(".//sru:recordData/marc:record/marc:datafield[@tag='245']",ns).getchildren():
            if subfield.attrib['code'] in ('a','b','n','p') :
                key_title = key_title + subfield.text
        return key_title

    def get_varying_form_of_title(self,record):
        out = []
        for title in record.findall(".//sru:recordData/marc:record/marc:datafield[@tag='246']/marc:subfield[@code='a']",ns):
            ET.dump(title)
            # if title.find(".//marc:subfield[@code='a']",ns) != None :
            #     self.logger.debug("test")
            out.append(title.text) 
        self.logger.debug("variante")
